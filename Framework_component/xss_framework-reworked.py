...
def test_xss_payload(method, url, headers, body, payload, proxies, timeout):
...
def xss_fm(method, url, headers, body, payloads, proxies=None, timeout=10, max_threads=50):
    ...
    def thread_func(payload):
        result = test_xss_payload(method, url, headers, body, payload, proxies, timeout)
        if result:
    ...
    for payload in payloads:
        ...
        t = threading.Thread(target=thread_func, args=(payload,))
        threads.append(t)
        t.start()
